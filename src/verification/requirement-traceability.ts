...
export const ACTIVE_REQUIREMENT_ID_PATTERN =
  /^(CONT|AUTHR|CINE|NARR|VOIC|CREW|VER|PROD|VIZ|RENDER)-\d+$/;
...
export const EXPECTED_V15_REQUIREMENT_IDS = [
  "RENDER-01",
  "RENDER-02",
  "RENDER-03",
  "RENDER-04",
...
/** @deprecated Use EXPECTED_V15_REQUIREMENT_IDS for active milestone validation. */
...
export function parseActiveRequirementIds(content: string): string[] {
  ...
  return [...ids].filter((id) =>
    EXPECTED_V15_REQUIREMENT_IDS.includes(id as (typeof EXPECTED_V15_REQUIREMENT_IDS)[number])
  );
}
...
export function validateRequirementTraceability(
  ...
  for (const requirementId of EXPECTED_V15_REQUIREMENT_IDS) {
  ...
  if (unmappedCount === 0 && activeIdsInTable.length !== EXPECTED_V15_REQUIREMENT_IDS.length) {
    errors.push({
      path: ".planning/REQUIREMENTS.md#traceability",
      reason: `Expected ${EXPECTED_V15_REQUIREMENT_IDS.length} v1.5 rows, found ${activeIdsInTable.length}.`
    });
  }
  ...
  return {
    errors,
    warnings,
    unmappedCount: unmappedCount === -1 ? EXPECTED_V15_REQUIREMENT_IDS.length : unmappedCount,
    pendingCount,
    rows
  };
}
