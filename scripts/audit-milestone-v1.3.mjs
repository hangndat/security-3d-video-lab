...
const AUDIT_OUT = resolve(REPO_ROOT, ".planning/milestones/v1.3-MILESTONE-AUDIT.md");
const ROADMAP_SNAPSHOT_OUT = resolve(REPO_ROOT, ".planning/milestones/v1.3-ROADMAP.md");
const ROADMAP_SOURCE = resolve(REPO_ROOT, ".planning/ROADMAP.md");

function snapshotRoadmap() {
  if (!existsSync(ROADMAP_SOURCE)) {
    throw new Error("Missing .planning/ROADMAP.md for v1.3 snapshot.");
  }
...
function main() {
  const report = buildV13MilestoneAuditReport(REPO_ROOT);
  mkdirSync(dirname(AUDIT_OUT), { recursive: true });
  writeFileSync(AUDIT_OUT, renderV13MilestoneAuditMarkdown(report), "utf-8");
  snapshotRoadmap();
